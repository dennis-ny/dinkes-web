...
<div class="w-full bg-white rounded shadow p-6">
    ...
    <form id="article-form" method="POST" action="{{ route('admin.berita.store') }}" enctype="multipart/form-data" class="space-y-5">
        @csrf

        {{-- JUDUL --}}
        <div>
            <label class="block text-sm font-medium mb-1">Judul Berita</label>
            <input type="text" name="judul" class="w-full border rounded px-3 py-2 focus:ring focus:ring-blue-200"
        ...
        <div>
            ...
            <input type="date" name="tanggal_terbit" class="w-full border rounded px-3 py-2 focus:ring focus:ring-blue-200"
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const oldContent = @json(old('konten'));
    if (oldContent) {
    ...
    function uploadFile(file, type) {
        ...
        let url = '{{ route("admin.berita.upload_image") }}';
        if (type === 'video') {
            url = '{{ route("admin.berita.upload_video") }}';
        }
